...
new Command('node-schematron')
			...
			.setResolver(
				(value) =>
					!value.length
						...
						: value.filter((name) => !!name).map((reporterName) => {
	...
	.addOption(new MultiOption('files').setShort('f').setDescription('A list of source files. ').isRequired(false))
	.addOption(
		new Option('batch')
			.setShort('b')
		...
		await gatherResultsFromChildProcesses(
			...
			(result, i) => {
				if (
					...
					// is used.
					!req.options.ok &&
					(result.$error || result.$value.some((assert) => !assert.isReport && assert.message))
				) {
					process.exitCode = 1;
				}
